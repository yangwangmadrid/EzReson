...
#
#    May 1, 2022:
#    - Add function DM_RAO_uni_spin() as anextension of DM_RAO_uni() for
#    spin-unrestricted cases
#
#    Feb 25, 2022:
#    - Add function popDM_spin() as anextension of popDM() for
#    spin-unrestricted cases
#
#    Jul 23, 2020:
...
# Gross atomic population and Wiberg bond orders, obtained from density matrix:
# ===== For SPIN-UNRESTRICED calculation only =====
# Note: the basis should be orthonormal set, such as NAOs, Lowdin AOs.
#   D:     Density matrix 
#   aoIx:  AO (forming the orthonormal basis) indices for each atom
#   q:     Gross atomic population
#   b:     BO-Valence matrix, corresponding to off-diagonal and diagonal entries
def popDM_spin( Da, Db, aoIx ):
    # Alpha:
    qa, BOa = popDM( Da, aoIx )
    BOa *= 2
    # Beta:
    qb, BOb = popDM( Db, aoIx )
    BOb *= 2
    # Total:
    q = qa + qb
    BO = BOa + BOb
...
# Spin-unrestricted version of DM_RAO_uni():
# Get density matrix for a SINGLE spin (either alpha or beta)
# Density matrix associtaed with a given Lewis structure in AO basis 
# (NAOs, Lowdin AOs, etc.), based on universal resonance atomic orbitals (RAOs) 
def DM_RAO_uni_spin( C_RAO, LP, BD, RA ):
    nLP = len( LP )
    if len( BD ) == 0:
        nBD = 0
    else:
        nBD = BD.shape[0]
    nRA = len( RA ) # Number of radical sites

    # Initialize density matrix in RAO basis:
    Drao = 0.5*np.eye( nLP + nRA + nBD*2 )

    for k in range( nLP ):
        # Density matrix in RAO basis (for a single spin):
        Drao[ k, k ] = 1 # Exact double occupancy (a lone pair)

    for k in range( nRA ):
        # Density matrix in RAO basis (for a single spin):
        Drao[ nLP+k, nLP+k ] = 1 # Exact double occupancy (a lone pair)

    atIx = list( np.array([*LP,*RA])-1 ) # A list to record index of atoms
    ic = nLP + nRA # A single variable to record appearance index of centers
    for ib in range( nBD ):
        atIx.append( BD[ib,0]-1 )
        atIx.append( BD[ib,1]-1 )
        # Density matrix in RAO basis (for closed-shell systems):
        Drao[ic,ic+1] = 0.5 # Exact covalent bond (equal sharing of electrons)
        Drao[ic+1,ic] = 0.5 # Exact covalent bond (equal sharing of electrons)
        ic += 2
    #writeMat( Drao, '/dev/stdout', ' %5.1f' )
...
def dmrt_spec( naoInfo, FchkInfo, CAONAO, CLMO, ELMO, atIx1, moIx1, ixLS, \
    ...
    # Calculate overlap matrix:
    print( 'Calculating overlap matrix between the Lewis structure '
            'density matrices...' )
